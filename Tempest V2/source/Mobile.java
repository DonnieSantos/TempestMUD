...

{
  ...
  public String getSClass()                     { return "Mb";                        }
  public String getCharClassColor()             { return "";                          }
  public String getSClassColor()                { return "#YMb#N";                    }
  ...
  {
    if (getName().length() == 0) return getName();
    return getName().substring(0,1).toUpperCase() + getName().substring(1,getName().length());
  }
...

  {
...

    {
      ...
      newMob.setInventory(new ItemList(INV_START_SIZE, ItemList.TYPE_INVENTORY));
...

  {
    ...
    if (getRoom().getN() != 0)
    if (getZone().getID() == World.getRoom(getRoom().getN()).getZone().getID())
    if (tryMove(DIRECTION_N).equals("OK")) {
      exits[ecount] = "N";
      ecount++; }

    if (getRoom().getS() != 0)
    if (getZone().getID() == World.getRoom(getRoom().getS()).getZone().getID())
    if (tryMove(DIRECTION_S).equals("OK")) {
      exits[ecount] = "S";
      ecount++; }

    if (getRoom().getE() != 0)
    if (getZone().getID() == World.getRoom(getRoom().getE()).getZone().getID())
    if (tryMove(DIRECTION_E).equals("OK")) {
      exits[ecount] = "E";
      ecount++; }

    if (getRoom().getW() != 0)
    if (getZone().getID() == World.getRoom(getRoom().getW()).getZone().getID())
    if (tryMove(DIRECTION_W).equals("OK")) {
      exits[ecount] = "W";
      ecount++; }

    if (getRoom().getU() != 0)
    if (getZone().getID() == World.getRoom(getRoom().getU()).getZone().getID())
    if (tryMove(DIRECTION_U).equals("OK")) {
      exits[ecount] = "U";
      ecount++; }

    if (getRoom().getD() != 0)
    if (getZone().getID() == World.getRoom(getRoom().getD()).getZone().getID())
